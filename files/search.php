...
<div class="containe-fluid">
	<?php 
		...
		$condition = $_SESSION['login_type'] == "1" || $_SESSION['login_type'] == '3' ? "" : "(f.is_public = 1 OR user_id ='$_SESSION[login_id]') AND";		
		$files = $conn->query("SELECT f.*,u.name as uname 
							   	FROM files f 
							   	inner join users u on u.id = f.user_id 
							   	where $condition file_status =0
							   		AND (f.name LIKE '%$_GET[file]%' OR tags LIKE '%$_GET[file]%' OR f.description LIKE '%$_GET[file]%') 
								UNION
								SELECT f.*,u.name as uname 
								FROM file_sharing 
								LEFT JOIN files f ON f.id = file_sharing.file_id
								LEFT JOIN users u ON f.user_id = u.id
								WHERE share_to_id='$_SESSION[login_id]' AND f.is_public <> 1 AND file_status =0 
									AND (f.name LIKE '%$_GET[file]%' OR tags LIKE '%$_GET[file]%' OR f.description LIKE '%$_GET[file]%')
