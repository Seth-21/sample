...
<nav aria-label="breadcrumb ">
  <ol class="breadcrumb">
  <li class="breadcrumb-item text-primary">Trash</li>
  </ol>
</nav>
<div class="containe-fluid">
	<?php include('db_connect.php') ;
	$files = $conn->query("SELECT f.*,u.name as uname FROM files f inner join users u on u.id = f.deleted_by where file_status =1 order by date(f.date_updated) desc");
				...
				<div class="card-body">
					<table width="100%">
						<tr>
							...
							<th width="30%" class="">Deleted By</th>
					...
					?>
						<tr class='file-item' data-id="<?php echo $row['id'] ?>" data-name="<?php echo $name ?>">
							...
							<td><button class="btn btn-sm btn-info btnRestore" id="<?php echo $row['id'] ?>"><i class="fa fa-trash-restore"></i></button></td>
						</tr>
...
<script>
	...
	$(".btnRestore").on('click', function(){
		...
		if(c){
			$.ajax({
				url:'ajax.php?action=file_restore',
				method:'POST',
				data:{id:$(this).attr('id')},
				success:function(resp){
					alert("File succesfully restored.");
					location.reload();
				}
			})
		}
